...
async def test_telegram_approve_own_group(db: AsyncSession):
    ...
    from app.bot.handlers import cmd_approve, BotContext
    ctx = BotContext(
        reply_chat_id="gl1_tg", actor_telegram_id="gl1_tg",
        actor_username=None, chat_type="private", message_thread_id=None,
        text=f"/approve {req.id}", command="/approve", args=[str(req.id)],
    )
    await cmd_approve(ctx, str(req.id), db)
...
async def test_telegram_approve_cross_group_rejected(db: AsyncSession):
    ...
    from app.bot.handlers import cmd_approve, BotContext
    ctx = BotContext(
        reply_chat_id="gl1_tg_x", actor_telegram_id="gl1_tg_x",
        actor_username=None, chat_type="private", message_thread_id=None,
        text=f"/approve {req.id}", command="/approve", args=[str(req.id)],
    )
    await cmd_approve(ctx, str(req.id), db)
...
async def test_telegram_deny_cross_group_rejected(db: AsyncSession):
    ...
    from app.bot.handlers import cmd_deny, BotContext
    ctx = BotContext(
        reply_chat_id="gl1_tg_d", actor_telegram_id="gl1_tg_d",
        actor_username=None, chat_type="private", message_thread_id=None,
        text=f"/deny {req.id} nope", command="/deny", args=[str(req.id), "nope"],
    )
    await cmd_deny(ctx, str(req.id), "nope", db)
...
async def test_telegram_requests_filters_by_group(db: AsyncSession):
    ...
    async def capture_send(chat_id: str, text: str, *, message_thread_id=None) -> None:
        sent.append(text)

    from app.bot import handlers as h
    ctx = h.BotContext(
        reply_chat_id="gl1_tg_list", actor_telegram_id="gl1_tg_list",
        actor_username=None, chat_type="private", message_thread_id=None,
        text="/requests", command="/requests", args=[],
    )
    with patch.object(h, "_send", side_effect=capture_send):
        await h.cmd_requests(ctx, db)
